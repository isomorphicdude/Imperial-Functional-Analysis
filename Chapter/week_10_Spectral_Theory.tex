...
\begin{example}\nl
    $T_\lambda$ is indicative of a certain class: $ X$ Hilbert, $T\in\mathcal{L}(X)$ compact, self-adjoint. \\
...
\begin{example}[Weak derivative]\nl
	...
	More precisely, $f\in\mathcal{H}$ is said to have a \textbf{weak derivative} $f'$ if $f'\defeq{v}$ for some $v\in \mathcal{H}$, 
...
\begin{proof}
  ...
  \end{center}
   ...
   % Short answer: we don't need boundedness. consider operator on $\ell^2$, that multiplies n-th term by n. This is unbounded, but for any $e_k$ it gives finite result.  
		...
		Back to (*): Due to (**), $M\defeq Im(\zeta-A)$ is closed. Assume $M\neq H$. Pick $v\in M^\perp\setminus\{0\}$. Then
		$$
