...
\begin{enumerate}[1)]
    ...
    Assume not, by \Cref{dual elements separate points}, $\exists \ell \in X^*: \ell(x) \neq \ell(y)$ if $x\neq y$. With this $\ell$: $$\ell(x) = \lim_{n \to \infty} \ell(x_n) = \ell(y) $$ 
    ,a contradiction.
    ...
    But by \Cref{dual characterisation of norm},  $||A_n||_{\mathcal{L}(X^*,\mathbb{R})} = \sup_{l\in X^*, ||\ell||\leq 1} |\ell(x_n)| = ||x_n||_X$.
\end{enumerate}
\end{remark}
...
Recall that we showed unit balls in $\infty$-dimensions are never (sequentially) compact (cf. \Cref{thm: unit ball not compact in infty dim}). Weak convergence allows us to restore a weak version of this .  
...
Together with \Cref{thm: Banach-Alaoglu} and \Cref{remark: reflexivity in Banach Alaoglu}, this gives immediately:
...
Unless $\dim H<\infty$, one \textbf{cannot} replace weak by strong in \Cref{corollary: hilbert weak convergence}.

\begin{example}
    \begin{enumerate}[i)]
        \item $X = L^1[0,1]$ is separable, $X^* \cong L^{\infty}$.
        If $(f_n) \subset L^{\infty}$ is bounded, i.e. $\sup \norm{f_n}_\infty < \infty$, \Cref{thm: Banach-Alaoglu} yields a subsequence $(n_k)_k \subset \natu$ and $f\in L^{\infty}$ s.t.  
        $$
        \lim_{k \to \infty} \int f_{n_k}g dx = \int fg dx, \qquad \forall g\in L^1
        $$
        \item $X = L^{\infty}(=L^{\infty}[0,1])$ is not separable (and also not reflexive). The following example shows that the conclusions of \Cref{thm: Banach-Alaoglu} fail in this case. 
    ...
    \item If instead consider $X = C^0[0,1]\subset L^{\infty}$, a separable closed subspace, then \Cref{thm: Banach-Alaoglu} applies to $T_\varepsilon \mid_X$. Indeed, one immediately sees that  
    $$
